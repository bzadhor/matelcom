...
$galleryRows = getDB()->prepare("SELECT image FROM produit_images WHERE produit_id=? ORDER BY ordre ASC, id ASC");
$galleryRows->execute([$prod['id']]);
$galleryUrls = [];
...
foreach ($galleryRows->fetchAll(PDO::FETCH_COLUMN) as $galleryImage) {
    $galleryUrl = imgUrl($galleryImage);
    if ($galleryUrl && !in_array($galleryUrl, $galleryUrls, true)) {
...
<style>
:root{--red:#D32F2F;--red-dark:#B71C1C;--red-light:#EF5350;--red-pale:#FFEBEE;--white:#FFF;--gray-50:#FAFAFA;--gray-100:#F5F5F5;--gray-200:#EEE;--gray-300:#E0E0E0;--gray-400:#BDBDBD;--gray-600:#757575;--gray-800:#424242;--gray-900:#212121;--dark:#1A1A2E;}
...
.produit-media{display:flex;flex-direction:column;gap:16px;}
.produit-image{background:white;border-radius:18px;border:1px solid var(--gray-200);padding:32px;display:flex;align-items:center;justify-content:center;min-height:380px;position:relative;cursor:zoom-in;}
...
.gallery-strip{display:grid;grid-template-columns:repeat(auto-fill,minmax(92px,1fr));gap:12px;}
.gallery-thumb{border:1px solid var(--gray-200);background:#fff;border-radius:14px;padding:8px;cursor:pointer;transition:border-color .2s,transform .2s;}
.gallery-thumb img{width:100%;height:76px;object-fit:contain;display:block;background:var(--gray-100);border-radius:10px;}
.gallery-thumb.active,.gallery-thumb:hover{border-color:var(--red);transform:translateY(-2px);}
.gallery-note{font-size:.8rem;color:var(--gray-600);}
...
.lightbox{position:fixed;inset:0;background:rgba(17,17,17,.88);display:none;align-items:center;justify-content:center;padding:24px;z-index:2000;}
.lightbox.open{display:flex;}
.lightbox-content{position:relative;max-width:min(92vw,1100px);max-height:90vh;width:100%;display:flex;align-items:center;justify-content:center;}
.lightbox-content img{max-width:100%;max-height:90vh;object-fit:contain;border-radius:18px;background:#fff;padding:16px;}
.lightbox-close{position:absolute;top:-12px;right:-12px;width:44px;height:44px;border:none;border-radius:999px;background:#fff;color:#111;font-size:1.1rem;cursor:pointer;box-shadow:0 10px 30px rgba(0,0,0,.25);}
...
@media(max-width:768px){.produit-grid{grid-template-columns:1fr;}.produit-image{min-height:300px;padding:20px;}.gallery-strip{grid-template-columns:repeat(auto-fill,minmax(78px,1fr));}}
</style>
</head>
<body>
<nav>
...
<div class="produit-page">
  ...
  <div class="produit-grid">
    <div class="produit-media">
      <div class="produit-image" <?= $mainImage ? 'data-zoom="'.$mainImage.'"' : '' ?>>
        <?php if ($mainImage): ?>
          <img id="mainProductImage" src="<?= e($mainImage) ?>" alt="<?= e($prod['nom']) ?>"/>
        <?php else: ?>
          <i class="fas fa-box-open" style="font-size:5rem;color:var(--gray-300)"></i>
        <?php endif; ?>
      ...
      <?php if ($galleryUrls): ?>
        <div class="gallery-strip">
          <?php foreach ($galleryUrls as $index => $galleryUrl): ?>
            <button type="button" class="gallery-thumb <?= $index === 0 ? 'active' : '' ?>" data-src="<?= e($galleryUrl) ?>" aria-label="Voir la photo <?= $index + 1 ?>">
              <img src="<?= e($galleryUrl) ?>" alt="<?= e($prod['nom']) ?> photo <?= $index + 1 ?>"/>
            </button>
          <?php endforeach; ?>
        </div>
        <div class="gallery-note">Cliquez sur une miniature pour changer la photo, puis cliquez sur l'image principale pour l'agrandir.</div>
      <?php endif; ?>
    </div>
  ...
  <div class="specs-section">
    <h2><i class="fas fa-list-alt" style="color:var(--red);margin-right:8px"></i> Caracteristiques techniques</h2>
  ...
  <div class="desc-long">
    <h2><i class="fas fa-info-circle" style="color:var(--red);margin-right:8px"></i> Description detaillee</h2>
...
<div class="lightbox" id="productLightbox" aria-hidden="true">
  <div class="lightbox-content">
    <button type="button" class="lightbox-close" id="lightboxClose" aria-label="Fermer"><i class="fas fa-times"></i></button>
    <img id="lightboxImage" src="" alt="<?= e($prod['nom']) ?>"/>
  </div>
</div>

<footer>
  <p>&copy; <?= date('Y') ?> <?= e($p['site_nom'] ?? 'MATELCOM') ?>. Tous droits reserves. <a href="index.php">Retour a l'accueil</a></p>
</footer>

<script>
var mainImage = document.getElementById('mainProductImage');
var imageFrame = document.querySelector('.produit-image');
var thumbs = document.querySelectorAll('.gallery-thumb');
var lightbox = document.getElementById('productLightbox');
var lightboxImage = document.getElementById('lightboxImage');
var closeLightbox = document.getElementById('lightboxClose');
...
thumbs.forEach(function (thumb) {
  thumb.addEventListener('click', function () {
    ...
    mainImage.src = src;
    imageFrame.setAttribute('data-zoom', src);
...
if (imageFrame && mainImage) {
  imageFrame.addEventListener('click', function () {
    var src = imageFrame.getAttribute('data-zoom');
    if (!src) return;
